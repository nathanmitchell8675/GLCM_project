...
f = open('/mnt/data1/ylee/for_Jason/20190523_seg_mrms_256_comp_real.bin','rb')
...
for n in range(num1, num2):
    ...
    #Declare feature
    feature = 'correlation'
    feature_values =  correlation_values
    ...
    for r in range (0, 256, tile_size):
        for c in range (0, 256, tile_size):
            ...
            rect = plt.Rectangle((x_offset, y_offset), tile_size, tile_size, 
                             facecolor='blue', edgecolor='none', alpha=1 -(correlation_values[(int(256/tile_size))*int(r/tile_size) + int(c/tile_size)] - correlation_min)/(correlation_max - correlation_min))
            ax[1].add_patch(rect)
    plt.show()
