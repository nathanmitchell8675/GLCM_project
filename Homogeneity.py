...
for n in range(num):
    ...
    feature = 'homogeneity'
    feature_values =  homogeneity_values
    ...
    for r in range (0, 256, tile_size):
        for c in range (0, 256, tile_size):
            ...
            rect = plt.Rectangle((x_offset, y_offset), tile_size, tile_size, 
                             facecolor='green', edgecolor='none', alpha=feature_values[(int(256/tile_size))*int(r/tile_size) + int(c/tile_size)])
            ax[1].add_patch(rect)
    fig.savefig(f'/home/nmitchell/texture/homogeneity_fig/Homogeneity_{isamp}.png')
#    plt.show()
    plt.close()
